...
const enrichUser = async (u) => {
  ...
  out.companies = (await UserCompany.find({ user_id: u._id })).map(
    (r) => r.company_id,
  );
  out.branches = (await UserBranch.find({ user_id: u._id })).map(
    (r) => r.branch_id,
  );
  out.departments = (await UserDepartment.find({ user_id: u._id })).map(
    (r) => r.department_id,
  );
  return out;
};
...
const grantSuperAdminAccess = async (userId) => {
  ...
  await UserCompany.insertMany(
    cos.map((c) => ({ user_id: userId, company_id: c._id })),
  );
  await UserBranch.insertMany(
    brs.map((b) => ({ user_id: userId, branch_id: b._id })),
  );
  await UserDepartment.insertMany(
    dpts.map((d) => ({ user_id: userId, department_id: d._id })),
  );
};
...
app.post("/api/users", auth, adminOnly, async (req, res) => {
  try {
    ...
    if (is_super_admin) {
      ...
      await UserCompany.insertMany(
        companies.map((c) => ({ user_id: id, company_id: c })),
      );
      await UserBranch.insertMany(
        branches.map((b) => ({ user_id: id, branch_id: b })),
      );
      await UserDepartment.insertMany(
        departments.map((d) => ({ user_id: id, department_id: d })),
      );
    }
...
app.put("/api/users/:id", auth, adminOnly, async (req, res) => {
  ...
  if (is_super_admin) {
    ...
    if (companies) {
      ...
      await UserCompany.insertMany(
        companies.map((c) => ({ user_id: req.params.id, company_id: c })),
      );
    }
    if (branches) {
      await UserBranch.deleteMany({ user_id: req.params.id });
      await UserBranch.insertMany(
        branches.map((b) => ({ user_id: req.params.id, branch_id: b })),
      );
    }
    if (departments) {
      await UserDepartment.deleteMany({ user_id: req.params.id });
      await UserDepartment.insertMany(
        departments.map((d) => ({ user_id: req.params.id, department_id: d })),
      );
    }
  }
  res.json({ ok: true });
...
app.get("/api/companies", auth, async (_req, res) => {
  ...
  const companiesWithBranches = await Promise.all(
    cos.map(async (c) => {
      const branches = await Branch.find({ company_id: c._id }).sort({
        name: 1,
      });
...
app.post("/api/companies", auth, adminOnly, async (req, res) => {
  ...
  await UserCompany.insertMany(
    admins.map((a) => ({ user_id: a._id, company_id: id })),
  );
...
app.post("/api/branches", auth, adminOnly, async (req, res) => {
  ...
  const admins = await User.find({ is_super_admin: true }, "_id");
  await UserBranch.insertMany(
    admins.map((a) => ({ user_id: a._id, branch_id: id })),
  );
...
app.post("/api/departments", auth, adminOnly, async (req, res) => {
  ...
  const admins = await User.find({ is_super_admin: true }, "_id");
  await UserDepartment.insertMany(
    admins.map((a) => ({ user_id: a._id, department_id: id })),
  );
...
app.get("/api/chat-groups", auth, async (req, res) => {
  const groups = await ChatGroup.find({})
    .populate({
      path: "branch_id",
      select: "name city avatar color company_id",
    })
    .populate({ path: "department_id", select: "name short_name icon color" })
    .sort({
      "branch_id.company_id": 1,
      "branch_id.name": 1,
      "department_id.name": 1,
    });
...
app.get("/api/conversations", auth, async (req, res) => {
  ...
  const result = await Promise.all(
    Array.from(conversationMap.entries()).map(async ([otherId, convo]) => {
